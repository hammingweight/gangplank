...
def get_drift_metrics(_X, Y):
    ...
    for y in Y:
        buckets[y.argmax()] += 1
    #print(buckets)
    #stats.chisquare(buckets)
    return gangplank.Drift(p_value=stats.chisquare(buckets).pvalue)
...
model = gangplank.PrometheusModel(model, port=8561, get_drift_metrics_func=get_drift_metrics)
